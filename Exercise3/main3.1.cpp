...
int main(int argc, char *argv[])
{
   ...
   ofstream Graf1;
   Graf1.open("CallDirect.out");
   if (!Graf1)
   {
      cerr << "Non si riesce ad aprire il file CallDirect!" << endl;
      return -1;
   }

   ofstream Graf2;
   Graf2.open("PutDirect.out");
   if (!Graf2)
   {
      cerr << "Non si riesce ad aprire il file PutDirect!" << endl;
      return -1;
   }
   ...
   //CALL-Option e PUT-Option directly
   for (int i=0; i<N; i++){
      ...
      for (int j = 0; j < L; j++){
         double S=So*exp((r - (pow(sigma,2) / 2))*T + sigma*rnd.Gauss(0, 1));
         Call += exp(-r * T) * mass(0, S - K); //serve un check sul segno di mass?
         Put += exp(-r * T) * mass(0, K - S);
      }

      av[i] = Call/L; // riempio il vettore delle medie
      avd[i] = Put/L;
   ...
   for (int i = 0; i < N; i++) {
      ...
      if (i == 0){ 
         Graf1 << 1 << "\t" << avc_prog[i] << "\t" << 0 << endl;
         Graf2 << 1 << "\t" << avp_prog[i] << "\t" << 0 << endl;
      }
      else{
         Graf1 << i+1 << "\t" << avc_prog[i] << "\t" << errc_barr[i] << endl;
         Graf2 << i+1 << "\t" << avp_prog[i] << "\t" << errp_barr[i] << endl;
      }
   };
