...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double gibbs = 1/(1 + exp(- (2*_J*_beta) * (_particle[this->pbc(i-1)].getspin() + _particle[this->pbc(i+1)].getspin())-2*_beta*_H));
      if (_rnd.Rannyu() < gibbs){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8.0*M_PI*_rho)/(9.0*pow(_r_cut, 9)) - (8.0*M_PI*_rho)/(3.0*pow(_r_cut, 3)); // TO BE FIXED IN EXERCISE 7 //epsilon=sigma=1
        ...
        index_property++;
        _ptail = (32.0*M_PI*_rho)/(9.0*pow(_r_cut, 9)) - (16.0*M_PI*_rho)/(3.0*pow(_r_cut, 3)); // TO BE FIXED IN EXERCISE 7
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // Controllo se la distanza è minore della metà del lato più corto della scatola di simulazione
        // Questo serve probabilmente per considerare le distanze minime tra le particelle
        if (dr < _halfside.min()){ 
          // Aggiunge la distanza al vettore per il calcolo della funzione di distribuzione radiale (GOFR)
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure) pressure_temp += 48*(1.0/pow(dr,12) - 0.5/pow(dr,6)); //PRESSURE, con epsilon e sigm=1, REDUCED UNITS           
        }
      }
    }
  }
  ...
  if (_measure_gofr){
    ...
    for (int i = 0; i < _n_bins; i++){
      ...
      _measurement(_index_gofr + i) = histogram[i] / (ideal_gas); //divido per _npart?
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      tenergy_temp /= double(_npart); //normalizzazione
...
// TO BE FIXED IN EXERCISE 6
  if (_measure_magnet){
    ...
    _measurement(_index_magnet) = magnetization / double(_npart); // send non divide per _npart
...
// TO BE FIXED IN EXERCISE 6 
  if (_measure_chi){
    double magnetization = 0.0; //a questo punto era diverso da zero quindi devo riazzerarlo
    for (int i = 0; i < _npart; i++) {
        magnetization += _particle(i).getspin();  // Somma dei momenti magnetici
    }
    double M2 = magnetization * magnetization;  // Quadrato della magnetizzazione totale
    _measurement(_index_chi) = _beta * M2 / double(_npart);
}
...
void System :: averages(int blk){
  ...
  //cose in meno dal send (non c'è average specifica per cv)
  _global_av  += _average;
