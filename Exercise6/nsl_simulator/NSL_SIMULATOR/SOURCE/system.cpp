...
void System :: read_configuration(){
  ...
  cinf.open("../INPUT/CONFIG/config" + format);
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & config" + format +" not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
...
void System :: averages(int blk){
  ...
  if (_measure_pressure){
    coutf.open("../OUTPUT/pressure_gas.dat",ios::app);
